...
class MyCounter(pysyncobj.SyncObj):
    def __init__(self, self_addr, other_addrs, **kwargs):
        super(MyCounter, self).__init__(self_addr, other_addrs, **kwargs)
        self._counter = 0
...
def main(argv=sys.argv[1:]):  # , stdin = sys.stdin):
    ...
    print('{} ready'.format(selfAddr), file=sys.stderr)
        ...
        if line == 'wait':
            ...
            while True:
                try:
                    ...
                    print('{} increment yielded SyncObjException with error code {}, retrying'.format(selfAddr,
                                                                                                      e.errorCode),
                          file=sys.stderr)
                else:
                    break
