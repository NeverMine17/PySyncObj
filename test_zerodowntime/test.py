...
parser.add_argument('revA',
                    help='path or git revision for the "old" version. When it is a path, '
                         'it must be the directory containing the pysyncobj package. '
                         'When it is a git revision, the parent directory of the directory containing this script'
                         ' must be the git repository, and this repository must contain the revision '
                         '(i.e. run this script from within the repository).')
parser.add_argument('revB', help='path or git revision for the "new" version')
parser.add_argument('cycles', nargs='?', type=int, default=120,
                    help='Number of cycles to run; must be at least ten times the number of processes')
parser.add_argument('processes', nargs='?', type=int, default=10,
                    help='Number of parallel processes; must be at least 3')
parser.add_argument('seed', nargs='?', type=int, default=None,
                    help='Seed for PRNG. Using the same seed value produces the exact same order of operations '
                         '*in this test script*, i.e. outside of PySyncObj. Everything inside the cluster, '
                         'e.g. which node is elected leader and when, is essentially still completely random.')
...
#  'compare' -- compare the value across all processes, verify that the majority has the same,
#               expected value; proc index is irrelevant in this case
...
# 60 % of the cycles must therefore be at least 5 times the number of processes,
# i.e. cycles >= 5/0.6 * processes = 8.33 * processes.
    ...
    with cd(tmpdirname):
        ...
        for revArg, revTarget in ((args.revA, 'revA'), (args.revB, 'revB')):
            if os.path.isdir(os.path.join(workingDir, revArg)):
                ...
                if subprocess.call(
                        ['rsync', '-a', os.path.join(workingDir, revArg, ''), os.path.join(revTarget, '')]) != 0:
                ...
                with cd(os.path.join(workingDir, '..')):  # TODO: Replace with GIT_DIR environment variable or something
                    ...
                    tarProc = subprocess.Popen(
                        ['tar', '-x', '-C', os.path.join(tmpdirname, revTarget), '--strip-components', '1',
                         'pysyncobj'], stdin=gitProc.stdout)
        ...
        for i in range(args.processes):
            ...
            if subprocess.call(
                    ['rsync', '-a', os.path.join('revA', ''), os.path.join('proc{}'.format(i), 'pysyncobj', '')]) != 0:
                print('rsync of revA to proc{} failed'.format(i), file=sys.stderr)
                sys.exit(1)
            if subprocess.call(
                    ['rsync', '-a', os.path.join(workingDir, 'proc.py'), os.path.join('proc{}'.format(i), '')]) != 0:
        ...
        try:
            ...
            for i in range(args.processes):
                with cd('proc{}'.format(i)):
                    procs.append(subprocess.Popen(
                        ['python3', 'proc.py', addrs[i]] + [addrs[j] for j in range(args.processes) if j != i],
                        stdin=subprocess.PIPE, stdout=subprocess.PIPE, bufsize=0))

            # Randomly run commands on the custer and upgrade the processes one-by-one, ensuring that everything's still
            # fine after each step
            counter = 0  # The expected value of the counter
            restart = -1  # Variable for when to restart a process; set to 3 on the 'upgrade' command, counted down on
            # each command, the upgraded process is restarted when it reaches zero
            ...
            for procId, command in commands:
                if command == 'increment':
                    ...
                    # Comparing the values of the counter doesn't work
                    # because the commands might not have been applied yet.
                    ...
                    for i in range(args.processes):
                        ...
                        procs[i].stdin.flush()
                    logs = [procs[i].stdout.readline().strip() if i != upgradingProcId else None for i in
                            range(args.processes)]

                    # Ensure that a majority of the logs are equal; note that this doesn't verify that all increments
                    # were actually replicated.
                    ...
                    # Copy revB
                    if subprocess.call(['rsync', '-a', os.path.join('revB', ''),
                                        os.path.join('proc{}'.format(procId), 'pysyncobj', '')]) != 0:
                ...
                if restart == 0:
                    ...
                    with cd('proc{}'.format(upgradingProcId)):
                        procs[upgradingProcId] = subprocess.Popen(
                            ['python3', 'proc.py', addrs[upgradingProcId]] + [addrs[j] for j in range(args.processes) if
                                                                              j != upgradingProcId],
                            stdin=subprocess.PIPE, stdout=subprocess.PIPE, bufsize=0)
            ...
            if not all(x == logs[0] for x in logs):
                ...
                print('ERROR: not all counters are equal to the expected value {}: {}'.format(counter, counters),
                      file=sys.stderr)
                status = 1
            else:
