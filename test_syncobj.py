...
from pysyncobj import SyncObj, SyncObjConf, replicated, FailReason, CommandType, \
    createJournal, HAS_CRYPTO, replicated_sync, Utility, SyncObjException, SyncObjConsumer, RaftState
import logging
import struct
import functools
...
def test_syncThreeObjectsLeaderFail():
    ...
    assert RaftState.LEADER in states[o1._getLeader().address]
    ...
    assert RaftState.LEADER in states[newObjs[0]._getLeader().address]
...
def test_randomTest1():
    ...
    while time.time() - st < 120.0:
        ...
        for i in xrange(0, random.randint(0, 2)):
            random.choice(objs).add_value(random.randint(0, 10))
        newObjs = list(objs)
...
def __checkParnerNodeExists(obj, nodeAddr, shouldExist=True):
    ...
    # either nodeAddr is in nodeAddrSet and shouldExist is True, or nodeAddr isn't in the set and shouldExist is False
    return (nodeAddr in nodeAddrSet) == shouldExist
...
def test_doChangeClusterUT1():
    ...
    noop = _bchr(CommandType.NO_OP)
    member = _bchr(CommandType.MEMBERSHIP)
...
def test_syncobjAdminSetVersion():
    ...
    assert o1.get_code_version() == 0
    assert o2.get_code_version() == 0
    ...
    doTicks([o1, o2], 10.0, stopFunc=lambda: o1.get_code_version() == 1 and o2.get_code_version() == 1)

    assert o1.get_code_version() == 1
    assert o2.get_code_version() == 1
...
def test_batteriesCommon():
    d1 = ReplDict()
    l1 = ReplLockManager(auto_unlock_time=30.0)

    d2 = ReplDict()
    l2 = ReplLockManager(auto_unlock_time=30.0)
    ...
    doAutoTicks(3.0, stopFunc=lambda: d1.get('testKey') is None)

    assert d1.get('testKey') is None

    assert l1.try_acquire('test.lock1', sync=True) == True
    assert l2.try_acquire('test.lock1', sync=True) == False
    assert l2.is_acquired('test.lock1') == False
    ...
    assert l2.try_acquire('test.lock1', sync=True) == True
...
def test_ReplList():
    ...
    assert l.raw_data() == [1, 2, 3]
    l.set(1, 10, _doApply=True)
    assert l.raw_data() == [1, 10, 3]
    l.append(42, _doApply=True)
    assert l.raw_data() == [1, 10, 3, 42]
    l.extend([5, 6], _doApply=True)
    assert l.raw_data() == [1, 10, 3, 42, 5, 6]
    l.insert(2, 66, _doApply=True)
    assert l.raw_data() == [1, 10, 66, 3, 42, 5, 6]
    l.remove(66, _doApply=True)
    assert l.raw_data() == [1, 10, 3, 42, 5, 6]
    l.pop(1, _doApply=True)
    assert l.raw_data() == [1, 3, 42, 5, 6]
    l.sort(reverse=True, _doApply=True)
    assert l.raw_data() == [42, 6, 5, 3, 1]
    assert l.index(6) == 1
    assert l.count(42) == 1
    assert l.get(2) == 5
    assert l[4] == 1
...
def test_ReplDict():
    ...
    assert d.pop(6, _doApply=True) is None
    ...
    assert d.items() == d.raw_data().items()
...
def test_ReplSet():
    ...
    assert s.raw_data() == {1, 4}

    s.add(10, _doApply=True)
    assert s.raw_data() == {1, 4, 10}
...
def test_ReplQueue():
    ...
    assert q.get(_doApply=True) is None
